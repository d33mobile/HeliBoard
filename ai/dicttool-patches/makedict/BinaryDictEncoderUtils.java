...
public class BinaryDictEncoderUtils {
    ...
    private static int getPtNodeMaximumSize(final PtNode ptNode,
            final HashMap<Integer, Integer> codePointToOneByteCodeMap) {
        return getPtNodeMaximumSize(ptNode, codePointToOneByteCodeMap, FormatSpec.VERSION202);
    }

    private static int getPtNodeMaximumSize(final PtNode ptNode,
            final HashMap<Integer, Integer> codePointToOneByteCodeMap, final int formatVersion) {
        int size = getNodeHeaderSize(ptNode, codePointToOneByteCodeMap);
        if (ptNode.isTerminal()) {
            // If terminal, one byte for the frequency.
            size += FormatSpec.PTNODE_FREQUENCY_SIZE;
        }
        // For children address (max possible width depends on format version)
        final int maxAddressSize = formatVersion >= FormatSpec.VERSION203
                ? FormatSpec.PTNODE_MAX_ADDRESS_SIZE_V203
                : FormatSpec.PTNODE_MAX_ADDRESS_SIZE;
        final int maxBigramAddressSize = formatVersion >= FormatSpec.VERSION203
                ? FormatSpec.PTNODE_ATTRIBUTE_MAX_ADDRESS_SIZE_V203
                : FormatSpec.PTNODE_ATTRIBUTE_MAX_ADDRESS_SIZE;
        size += maxAddressSize;
        if (null != ptNode.mBigrams) {
            size += (FormatSpec.PTNODE_ATTRIBUTE_FLAGS_SIZE + maxBigramAddressSize)
    ...
    private static void calculatePtNodeArrayMaximumSize(final PtNodeArray ptNodeArray,
            final HashMap<Integer, Integer> codePointToOneByteCodeMap) {
        calculatePtNodeArrayMaximumSize(ptNodeArray, codePointToOneByteCodeMap,
                FormatSpec.VERSION202);
    }
    ...
    static int getByteSize(final int address) {
        return getByteSize(address, FormatSpec.VERSION202);
    }

    /**
     * VERSION203 extension: when address > UINT24_MAX-1 (i.e. needs the 0xFFFFFF
     * sentinel), encode as 7 bytes (3 sentinel + 4 extended). For VERSION202,
     * fail loud instead of silently truncating like the original encoder did.
     */
    static int getByteSize(final int address, final int formatVersion) {
        if (!BinaryDictIOUtils.hasChildrenAddress(address)) {
            return 0;
        }
        final int abs = Math.abs(address);
        if (abs <= FormatSpec.UINT8_MAX) return 1;
        if (abs <= FormatSpec.UINT16_MAX) return 2;
        if (abs < FormatSpec.UINT24_MAX) return 3;  // < 0xFFFFFF: regular 3-byte
        // abs >= 0xFFFFFF: needs extended encoding (sentinel reserved)
        if (formatVersion >= FormatSpec.VERSION203) return 7;
        throw new RuntimeException("address " + address + " >= 16 MB cannot be encoded "
                + "in VERSION" + formatVersion + "; use VERSION203 (-203 flag)");
    }
    ...
    static int writeUIntToBuffer(final byte[] buffer, final int fromPosition, final int value,
        ...
        switch(size) {
            case 7:
                // VERSION203 extended: 3 sentinel bytes (0xFF) + 4 bytes BE value
    ...
    private static boolean computeActualPtNodeArraySize(final PtNodeArray ptNodeArray,
            ...
            final HashMap<Integer, Integer> codePointToOneByteCodeMap) {
        return computeActualPtNodeArraySize(ptNodeArray, dict, codePointToOneByteCodeMap,
                FormatSpec.VERSION202);
    }
        ...
        for (PtNode ptNode : ptNodeArray.mData) {
            ...
            if (null != ptNode.mChildren) {
                nodeSize += getByteSize(getOffsetToTargetNodeArrayDuringUpdate(ptNodeArray,
                        nodeSize + size, ptNode.mChildren), formatVersion);
            }
            if (null != ptNode.mBigrams) {
                for (WeightedString bigram : ptNode.mBigrams) {
                    ...
                    nodeSize += getByteSize(offset, formatVersion)
    ...
    /* package */ static ArrayList<PtNodeArray> computeAddresses(final FusionDictionary dict,
            final ArrayList<PtNodeArray> flatNodes,
            final HashMap<Integer, Integer> codePointToOneByteCodeMap) {
        return computeAddresses(dict, flatNodes, codePointToOneByteCodeMap, FormatSpec.VERSION202);
    }
    ...
    /* package */ static ArrayList<PtNodeArray> computeAddresses(final FusionDictionary dict,
        ...
        for (final PtNodeArray n : flatNodes) {
            calculatePtNodeArrayMaximumSize(n, codePointToOneByteCodeMap, formatVersion);
        }
        ...
        do {
            ...
            for (final PtNodeArray ptNodeArray : flatNodes) {
                ...
                final boolean changed = computeActualPtNodeArraySize(ptNodeArray, dict,
                        codePointToOneByteCodeMap, formatVersion);
    ...
    /* package */ static int writeChildrenPosition(final byte[] buffer, final int fromIndex,
            final int position) {
        return writeChildrenPosition(buffer, fromIndex, position, FormatSpec.VERSION202);
    }

    /* package */ static int writeChildrenPosition(final byte[] buffer, final int fromIndex,
            final int position, final int formatVersion) {
        int index = fromIndex;
        switch (getByteSize(position, formatVersion)) {
        case 1:
        ...
        case 3:
            ...
            return 3;
        case 7:
            // VERSION203 extended: 3 sentinel bytes (0xFF) + 4 bytes BE position
            buffer[index++] = (byte) 0xFF;
            buffer[index++] = (byte) 0xFF;
            buffer[index++] = (byte) 0xFF;
            buffer[index++] = (byte)(0xFF & (position >> 24));
            buffer[index++] = (byte)(0xFF & (position >> 16));
            buffer[index++] = (byte)(0xFF & (position >> 8));
            buffer[index++] = (byte)(0xFF & position);
            return 7;
    ...
    static int makePtNodeFlags(final boolean hasMultipleChars, final boolean isTerminal,
        ...
        switch (childrenAddressSize) {
            ...
            case 3:
            case 7:
                // VERSION203: case 7 (sentinel + 4-byte ext) presents as
                // THREEBYTES at the flag-byte level. The decoder spots the
                // sentinel inside the address bytes and switches to extended.
    ...
    /* package */ static byte makePtNodeFlags(final PtNode node, final int childrenOffset) {
        return makePtNodeFlags(node, childrenOffset, FormatSpec.VERSION202);
    }

    /* package */ static byte makePtNodeFlags(final PtNode node, final int childrenOffset,
            final int formatVersion) {
        return (byte) makePtNodeFlags(node.mChars.length > 1, node.isTerminal(),
                getByteSize(childrenOffset, formatVersion),
    ...
    /* package */ static int makeBigramFlags(final boolean more, final int offset,
        ...
        return makeBigramFlags(more, offset, bigramFrequency, unigramFrequency, word,
                FormatSpec.VERSION202);
    }
    ...
    /* package */ static int makeBigramFlags(final boolean more, final int offset,
        ...
        switch (getByteSize(offset, formatVersion)) {
        case 1:
        ...
        case 3:
        case 7:
            // VERSION203: case 7 (sentinel + 4-byte ext) presents as THREEBYTES
            // at the bigram-flag level. Decoder spots sentinel and reads ext.
