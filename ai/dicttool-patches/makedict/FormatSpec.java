...
public final class FormatSpec {
    ...
    // VERSION203: extends VERSION202 children/bigram address with sentinel-based
    // 4-byte extended addressing. When 3-byte address reads as 0xFFFFFF, the
    // following 4 bytes are the actual 32-bit (unsigned) address. Allows trie
    // size up to 4 GiB. Backward-compatible read path: v202 dicts produced
    // before this never used 0xFFFFFF as a valid address (assert tripped at
    // 16 MB), so v203-aware decoder reads both formats safely.
    ...
    // VERSION203 extended children/bigram address: 3 sentinel bytes (0xFFFFFF) + 4 bytes BE
    static final int PTNODE_MAX_ADDRESS_SIZE_V203 = 7;
    static final int PTNODE_ATTRIBUTE_FLAGS_SIZE = 1;
    static final int PTNODE_ATTRIBUTE_MAX_ADDRESS_SIZE = 3;
    static final int PTNODE_ATTRIBUTE_MAX_ADDRESS_SIZE_V203 = 7;
